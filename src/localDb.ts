 ...
 * Replaces Firebase Firestore with localStorage
 */
...
export function addData<T extends DataRecord>(collectionName: string, userId: string, item: Omit<T, 'id' | 'uid'>): T {
    const data = getCollection<T>(collectionName);
    const newItem = {
        ...item,
        id: generateId(),
...
export function updateData<T extends DataRecord>(collectionName: string, id: string, updates: Partial<T>): void {
    const data = getCollection<T>(collectionName);
