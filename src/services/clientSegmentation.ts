...
export class ClientSegmentationService {
  ...
  analyzeClient(client: Client, orders: Order[], settings: AppSettings): ClientSegment {
    ...
    const daysSinceLastOrder = lastOrder ? Math.floor((Date.now() - new Date(lastOrder.date).getTime()) / (1000 * 60 * 60 * 24)) : Infinity;
  ...
  getClientSegmentationDetails(client: Client, orders: Order[], settings: AppSettings): ClientSegmentationData {
  ...
  getSegmentStats(clients: Client[], orders: Order[], settings: AppSettings): SegmentStats[] {
    ...
    clients.forEach(client => {
      ...
      const lastOrder = clientOrders.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
      if (lastOrder && (!lastOrderDates[segment] || new Date(lastOrder.date) > (lastOrderDates[segment] || new Date(0)))) {
  ...
  getClientsBySegment(segment: ClientSegment, clients: Client[], orders: Order[], settings: AppSettings): Client[] {
