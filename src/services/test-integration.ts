...
const testClients: Client[] = [
  { id: 'c1', name: 'Jonas Petraitis', phone: '+37060000001', address: 'Klaipėda, Baltijos pr. 1', buildingType: 'butas', createdAt: '2024-01-15' },
  { id: 'c2', name: 'UAB "Baltija"', phone: '+37060000002', address: 'Klaipėda, Tilžės g. 25', buildingType: 'ofisas', createdAt: '2024-02-20' },
  { id: 'c3', name: 'Ona Kazlauskienė', phone: '+37060000003', address: 'Klaipėda, Bangų g. 8', buildingType: 'butas', createdAt: '2024-01-10' }
];

const testOrders: Order[] = [
  { id: 'o1', clientId: 'c1', clientName: 'Jonas Petraitis', address: 'Klaipėda, Baltijos pr. 1', date: '2024-03-15', time: '10:00', windowCount: 8, floor: 3, additionalServices: { balkonai: true, vitrinos: false, terasa: false, kiti: false }, totalPrice: 45, status: 'atlikta', createdAt: '2024-03-10', isPaid: false },
  { id: 'o2', clientId: 'c2', clientName: 'UAB "Baltija"', address: 'Klaipėda, Tilžės g. 25', date: '2024-03-20', time: '09:00', windowCount: 25, floor: 2, additionalServices: { balkonai: false, vitrinos: true, terasa: false, kiti: false }, totalPrice: 120, status: 'suplanuota', createdAt: '2024-03-18', isPaid: true },
  { id: 'o3', clientId: 'c3', clientName: 'Ona Kazlauskienė', address: 'Klaipėda, Bangų g. 8', date: '2024-02-01', time: '14:00', windowCount: 5, floor: 1, additionalServices: { balkonai: false, vitrinos: false, terasa: false, kiti: false }, totalPrice: 30, status: 'atlikta', createdAt: '2024-01-28', isPaid: true }
];

const testExpenses: Expense[] = [
  { id: 'e1', title: 'Langų valiklis', amount: 25.50, date: '2024-03-01', category: 'priemonės', uid: 'user1' },
  { id: 'e2', title: 'Kuras', amount: 45.00, date: '2024-03-05', category: 'kuras', uid: 'user1' },
  { id: 'e3', title: 'Facebook reklama', amount: 100.00, date: '2024-03-10', category: 'reklama', uid: 'user1' }
];
...
async function testMemoryPriority(): Promise<{ passed: boolean; details: string }> {
  try {
    ...
    const isSorted = prioritized.every((mem, i, arr) => 
      i === 0 || arr[i-1].priorityScore >= mem.priorityScore
    );
    ...
    console.log(`  🏆 Top memory: "${prioritized[0].content.substring(0, 50)}..." (${(prioritized[0].priorityScore * 100).toFixed(0)}%)`);
...
async function testHybridClassifier(apiKey?: string): Promise<{ passed: boolean; details: string }> {
  try {
    const testQueries = [
    ...
    for (const { query, expectedCategory } of testQueries) {
      const result = apiKey 
        ...
        : { intention: 'general_chat' as any, confidence: 0.5, method: 'keyword' as const, shouldExecuteTool: false, toolName: null, parameters: {}, alternatives: [] };
      
      console.log(`  📝 Query: "${query}" → ${result.intention} (${result.method}, ${(result.confidence * 100).toFixed(0)}%)`);
    ...
    if (accuracy >= 0.5) {
      ...
      return { passed: false, details: `Classifier accuracy too low: ${(accuracy * 100).toFixed(0)}%` };
    }
  } catch (error) {
...
async function testModularPrompt(): Promise<{ passed: boolean; details: string }> {
  try {
    ...
    console.log(`  ✅ Modular prompt assembled: ${assemblyResult.modules.length} modules, ${assemblyResult.totalTokens} tokens`);
    console.log(`  📦 Sections: ${assemblyResult.modules.map(m => m.type).join(', ')}`);
    
    return { passed: true, details: `Prompt assembly successful: ${assemblyResult.totalTokens} tokens` };
  } catch (error) {
...
async function testPlanningEngine(): Promise<{ passed: boolean; details: string }> {
  try {
    ...
    if (summary.status === 'failed' && summary.failedSteps > 0) {
      return { passed: false, details: `Plan execution failed: ${summary.failedSteps} steps failed` };
    }
    ...
    return { passed: true, details: `Planning engine working: ${summary.completedSteps} steps completed` };
  } catch (error) {
    return { passed: false, details: `Planning engine error: ${error}` };
  }
}

async function testEndToEndWorkflow(apiKey?: string): Promise<{ passed: boolean; details: string }> {
  try {
    ...
    console.log(`  2️⃣ Classification: ${classification.intention} (${classification.method}, ${(classification.confidence * 100).toFixed(0)}%)`);
    ...
    console.log(`  4️⃣ Prompt assembled: ${assemblyResult.totalTokens} tokens, ${assemblyResult.modules.length} modules`);
