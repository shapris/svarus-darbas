...
async function runKeywordTests(): Promise<{ passed: number; failed: number; total: number }> {
  ...
  for (const testCase of KEYWORD_TEST_CASES) {
    ...
    try {
      ...
      if (result.intention === testCase.expectedIntent && result.confidence >= testCase.minConfidence) {
        console.log('  ✅ PASSED');
        passed++;
      } else {
        console.log(`  ❌ FAILED - Expected ${testCase.expectedIntent} with confidence >= ${testCase.minConfidence}`);
        failed++;
      }
    } catch (error) {
...
async function runLLMTests(apiKey?: string): Promise<{ passed: number; failed: number; total: number }> {
  if (!apiKey) {
...
async function runEdgeCaseTests(apiKey?: string): Promise<{ passed: number; failed: number; total: number }> {
  let passed = 0;
  let failed = 0;
...
async function simulateKeywordClassification(text: string): Promise<{
  ...
  if (/pagalbos|help|ką.*gali/i.test(textLower)) {
    return { intention: 'help_request', confidence: 0.90 };
  }
  ...
  if (/top.*klient|geriausi.*klient|vip.*klient/i.test(textLower)) {
    return { intention: 'vip_clients', confidence: 0.90 };
  }
...
export async function performanceTest(
  ...
  console.log(`Keyword classifications: ${keywordCount} (${(keywordTime / Math.max(keywordCount, 1)).toFixed(2)}ms avg)`);
  console.log(`LLM classifications: ${llmCount} (${(llmTime / Math.max(llmCount, 1)).toFixed(2)}ms avg)`);
