...
export const SMS_TEMPLATES: SMSTemplate[] = [
  {
    ...
    template: 'Sveiki {vardas}, primename apie langų valymą {data} {laikas}. Adresas: {adresas}. Kaina: {kaina}. Iki pasimatymo!',
    description: 'Siunčiama 24h prieš valymą'
  },
  {
    id: 'reminder_1h',
    name: 'Priminimas prieš 1 valandą',
    template: 'Sveiki {vardas}, langų valymas prasideda po valandos! {data} {laikas}. Adresas: {adresas}. Laukiame jus!',
    description: 'Siunčiama 1h prieš valymą'
  },
  {
    id: 'thank_you',
    name: 'Ačiū už paslaugą',
    template: 'Sveiki {vardas}, ačiū, kad pasirinkote mūsų paslaugas! Laukiame grįžtant. Atsiliepimus galite palikti: {atsiliepimu_linkas}',
    description: 'Siunčiama po atliktų darbų'
  },
  {
    id: 'payment_reminder',
    name: 'Mokėjimo priminimas',
    template: 'Sveiki {vardas}, primename apie neapmokėtą sąskaitą už langų valymą {data}. Suma: {kaina}. Mokėjimo informacija: {mokejimo_info}',
...
export class SMSService {
    ...
    this.reminders.forEach(reminder => {
      if (!reminder.sent && reminder.scheduledFor <= now) {
        ...
        if (order && client && client.phone) {
          const template = SMS_TEMPLATES.find(t => t.id === `${reminder.type}_reminder`) || 
  ...
  getTodaysReminders(): SMSReminder[] {
    ...
    return this.reminders.filter(r => 
      r.scheduledFor >= startOfDay && r.scheduledFor < endOfDay
    );
  }

  // Manual send reminder
  sendManualReminder(orderId: string, type: '24h' | '1h' | 'thank_you' | 'payment', 
                     orders: Order[], clients: Client[], settings: AppSettings): void {
    const order = orders.find(o => o.id === orderId);
    const client = clients.find(c => c.id === order?.clientId);
    
    if (order && client && client.phone) {
      const template = SMS_TEMPLATES.find(t => t.id === `${type}_reminder`) || 
