...
export class AnalyticsService {
  ...
  getAnalytics(orders: Order[], clients: Client[], expenses: Expense[], settings: AppSettings): AnalyticsData {
    const now = new Date();
    ...
    const revenue = {
      ...
      thisMonth: this.calculateRevenue(completedOrders.filter(o => new Date(o.date) >= thisMonth)),
      lastMonth: this.calculateRevenue(completedOrders.filter(o => {
    ...
    // Performance metrics
    const performance = {
      avgOrderValue: completedOrders.length > 0 ? revenue.thisMonth / completedOrders.filter(o => new Date(o.date) >= thisMonth).length : 0,
  ...
  // Get top performers
  getTopPerformers(orders: Order[], clients: Client[]): {
  ...
  // Get forecast data
  getForecast(orders: Order[], settings: AppSettings): {
