        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              {todaysOrders.map((order, index) => {
                ...
                return (
                  ...
                  >
                    {/* Timeline Dot */}
                    <div className={`absolute left-4 top-4 w-6 h-6 rounded-full border-4 border-slate-50 flex items-center justify-center ${
                    ...
                    {/* Order Card */}
                    <div className={`bg-white p-5 rounded-3xl border ${isCompleted ? 'border-emerald-100 opacity-75' : 'border-slate-100 shadow-sm'}`}>
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-2">
                          <Clock size={16} className={isCompleted ? 'text-emerald-500' : 'text-blue-500'} />
                          <span className="font-black text-slate-900">{order.time}</span>
                        </div>
                        <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-lg ${
                          isCompleted ? 'bg-emerald-50 text-emerald-600' : 
                          isPending ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600'
                        }`}>
                          {order.status}
                        </span>
                      </div>
          ...
          <div className="h-[600px] bg-slate-100 rounded-3xl overflow-hidden border border-slate-200 shadow-sm sticky top-6 z-0">
            ...
            >
              ...
              />
              {todaysOrders.filter(o => o.lat && o.lng).map((order, index) => (
                <Marker 
                  key={order.id} 
                ...
                >
                  <Popup>
                    <div className="font-sans">
                      <div className="font-bold text-sm mb-1">{order.time} - {order.clientName}</div>
                      <div className="text-xs text-slate-500">{order.address}</div>
                      <div className="text-xs font-semibold mt-1 text-blue-600">{order.status}</div>
                    </div>
                  </Popup>
                </Marker>
              ))}
