/**
 * Šis puslapis atidaromas iš el. laiško nuorodos (Supabase slaptažodžio atkūrimas).
 */

import React, { useState } from 'react';
import { Droplets, Lock } from 'lucide-react';
import { motion } from 'motion/react';
import { supabase, usesLocalStorageBackend, needsBackendSetup } from '../supabase';
    ...
    if (usesLocalStorageBackend || needsBackendSetup || !supabase) {
      setError(
        usesLocalStorageBackend
          ? 'Slaptažodžio keitimas nepasiekiamas vietiniame kūrimo režime be debesies.'
          : 'Slaptažodžio keitimas negalimas: nesukonfigūruota debesies autentifikacija.'
      );
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
      ...
      >
        ...
        <p className="text-slate-600 text-sm text-center mb-6">
          Įveskite naują slaptažodį paskyrai „Supabase“.
        </p>

        {done ? (
          <p className="text-center text-emerald-700 text-sm font-medium">
            Slaptažodis atnaujintas. Nukreipiama…
          </p>
        ) : (
